...
"""Finetuning the library models for sequence classification on GLUE (Bert, XLM, XLNet, RoBERTa)
   using DistributedDataParallel (DDP). In this version, we run for 1 epoch,
   discard the timing of the first iteration, and report the average iteration time.
...
def train(args, train_dataset, model, tokenizer):
    """Train the model for 1 epoch using DDP.
       For each iteration (except the first) we record the time,
       and at the end we log the average iteration time.
    """
    ...
    all_iter_times = []  # To collect average iteration time per epoch.
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = outputs[0]  # First element is the loss.
            ...
            if args.fp16:
                ...
                # No manual gradient synchronization is needed with DDP.
                loss.backward()
            ...
            # Discard the timing of the first iteration.
            if step > 0:
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        if task in ['mnli', 'mnli-mm'] and args.model_type in ['roberta']:
            # For RoBERTa, label indices are swapped.
...
def main():
    ...
    parser.add_argument("--total_batch_size", type=int, default=64,
                        help="Total batch size (per-worker batch size * number of workers), should equal 64 for Tasks 2/3")
    ...
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Only process 0 downloads the model & vocab.
