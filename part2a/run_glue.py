...
def train(args, train_dataset, model, tokenizer):
    ...
    # Verify that total batch size = per-device batch size * number of workers.
    ...
    # We'll collect iteration timings (excluding the first iteration) in this list.
    all_iter_times = []
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.fp16:
                ...
                # Backward pass
                loss.backward()
                # Manual gradient synchronization using gather and scatter:
                for param in model.parameters():
                    ...
                    # On worker 0, compute the average.
                    if args.local_rank == 0:
                    ...
                    if args.local_rank == 0:
                        scatter_list = [grad_avg for _ in range(args.world_size)]
                    else:
                        scatter_list = None
            ...
            tr_loss += loss.item()
            # Only record timing if this is not the first iteration.
            end_time = time.time()
            iter_time = end_time - start_time
            if step > 0:
                iter_times.append(iter_time)
            # Log per-iteration loss and time.
    ...
    # Optionally, log overall average iteration time (if more than one epoch).
    if all_iter_times:
...
def evaluate(args, model, tokenizer, prefix=""):
    # Handles evaluation (for MNLI double evaluation, etc.)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # Use SequentialSampler for evaluation.
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Ensure only the first process downloads the dataset
...
def main():
    ...
    # Set num_train_epochs to 1 per Task 2,3 instructions.
    ...
    # Load the model using from_pretrained with config.
